...
def filter_out!(arr, &prc)
    idx = 0
    prc.call(arr[idx]) ? arr.delete_at(idx) : idx += 1 while idx < arr.length
end
...
def proctition(arr, &prc)
    arr.select { |ele| prc.call(ele) } + arr.reject { |ele| prc.call(ele) }
end
...
def proc_suffix(str, h)
    ...
    new_str = words.map do |word|
        new_word = word
        h.each { |k, v| new_word += v if k.call(word) }
        new_word
    end
    new_str.join(" ")
end
...
def proctition_platinum(arr, *procs)
    h = Hash.new { |h, k| h[k] = [] }
    procs.each.with_index do |prc, idx|
        h[idx + 1] = arr.select { |ele| prc.call(ele) }
        h[idx + 1].each { |ele| arr.delete(ele) }
    end
    h
end
...
def procipher(str, h)
    ...
    new_str = words.map do |word|
        new_word = word
        h.each { |k, v| new_word = v.call(word) if k.call(word) }
        new_word
    end
    new_str.join(" ")
end
